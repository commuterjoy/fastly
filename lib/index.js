 ...
 * Constructor
 */
function Fastly (apikey, service) {
            ...
            self[a.fn] = function () {
                ...
                var interpolateUrl = function (url, tokens) {
                    return url
                            .split('/')
                            .map( function (a) {
                                return /^\%/.test(a) ? tokens.pop() : a; // pop. not idempotent. sue me.
                            })
                            .join('/')
                }

                var args = [].slice.call(arguments).reverse().concat([self.service]);
                var endPoint = interpolateUrl(a.url, args);
 ...
 * Export
 */
module.exports = function (apikey, service) {
    return new Fastly(apikey, service);
};
